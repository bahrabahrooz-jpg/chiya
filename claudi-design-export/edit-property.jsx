...
const { Icon, Button, IconButton, Avatar, Badge, Input, Textarea, Select, Notice, Modal } = DS;

const LOGO = "assets/chiya-logomark.svg";

/* Shared, cross-component guard so the shell (sidebar / topbar) can defer a
   navigation while the Edit Property form has unsaved changes. The form
   registers `requestLeave`; the shell calls `dirty()` before navigating. */
const leaveGuard = {
  dirty: () => false,
  requestLeave: (href) => { window.location.href = href; },
};
...
/* Custom dropdown — matches the web design's listbox (Locations page):
   styled trigger, floating panel, green-highlighted selected row + check.
   Portaled to <body> so the card's overflow:hidden never clips it. */
function Dropdown({ id, options, value, onChange, placeholder }) {
  ...
  const calc = () => {
    ...
    setCoords({ top, left: r.left, width: r.width, clipTop: Math.max(0, navBottom - top) });
  };
  const openDropdown = () => { calc(); setOpen(true); };
  ...
  return (
    <>
      <button ref={triggerRef} type="button" id={id}
        className={["lc-dd__trigger", open && "is-open"].filter(Boolean).join(" ")}
        onClick={() => (open ? setOpen(false) : openDropdown())}
        aria-haspopup="listbox" aria-expanded={open}>
        {selected
          ? <span className="lc-dd__val"><span className="lc-dd__val-txt">{selected.label}</span></span>
      ...
      {open && coords && ReactDOM.createPortal(
        <div ref={panelRef} className="lc-dd__panel"
          style={{ top: coords.top, left: coords.left, width: coords.width,
            clipPath: coords.clipTop ? `inset(${coords.clipTop}px 0 0 0)` : undefined }} role="listbox">
          <div className="lc-dd__scroll">
            {options.map((o) => (
              <button key={o.value || "__empty"} type="button"
                className={"lc-dd__row" + (value === o.value ? " is-selected" : "")}
                role="option" aria-selected={value === o.value}
                onClick={() => { onChange(o.value); setOpen(false); }}>
                <span className="lc-dd__body"><span className="lc-dd__row-title">{o.label}</span></span>
...
function ComboSelect({ side, value, onChange, options }) {
  ...
  const calc = () => {
    ...
    const navBottom = nav ? nav.getBoundingClientRect().bottom : 0;
    const top = r.bottom + 8;
    setCoords({ top, left: r.left, right: window.innerWidth - r.right, clipTop: Math.max(0, navBottom - top) });
  };
  const openDropdown = () => { calc(); setOpen(true); };
  ...
  return (
    <div className={"ap-combo__sel ap-combo__sel--" + side}>
      <button ref={triggerRef} type="button"
        className={"ap-combo__sel-btn" + (open ? " is-open" : "")}
        onClick={() => (open ? setOpen(false) : openDropdown())}
        aria-haspopup="listbox" aria-expanded={open} aria-label="Unit">
        {value}
      </button>
      <span className={"ap-combo__sel__chev" + (open ? " is-open" : "")}><Icon name="chevron-down" size={16} /></span>

      {open && coords && ReactDOM.createPortal(
        <div ref={panelRef} className="lc-dd__panel ap-combo__panel"
          style={{
            ...(side === "right"
              ? { top: coords.top, right: coords.right }
              : { top: coords.top, left: coords.left }),
            clipPath: coords.clipTop ? `inset(${coords.clipTop}px 0 0 0)` : undefined,
          }} role="listbox">
          <div className="lc-dd__scroll">
            {options.map((o) => (
              <button key={o} type="button"
                className={"lc-dd__row" + (value === o ? " is-selected" : "")}
                role="option" aria-selected={value === o}
                onClick={() => { onChange(o); setOpen(false); }}>
                <span className="lc-dd__body"><span className="lc-dd__row-title">{o}</span></span>
...
================================================================== */
const COVER_IMG = "https://images.unsplash.com/photo-1613490493576-7fde63acd811?auto=format&fit=crop&w=1280&q=70";
const GALLERY_IMGS = [
...
function VideoUpload() {
  return (
    <div className="ap-vid ap-vid--done">
      <div className="ap-vid__row">
        ...
        <div className="ap-vid__meta">
          ...
          <span className="ap-vid__sub">84.2 MB · 2:14 · Uploaded</span>
        </div>
        <span className="ap-vid__badge"><Icon name="check" size={13} strokeWidth={3} />Uploaded</span>
      </div>
      <div className="ap-vid__actions">
        <button type="button" className="ap-vid__act"><Icon name="repeat-2" size={15} strokeWidth={2.25} />Replace video</button>
        <button type="button" className="ap-vid__act ap-vid__act--danger"><Icon name="trash-2" size={15} strokeWidth={2.25} />Remove</button>
      </div>
    </div>
  );
}
...
/* Custom agent picker — mirrors the Members modal agent dropdown
   (avatar + verified ring, name, area subtitle, green check on selected). */
function AgentSelect({ id, value, onChange }) {
  const [open, setOpen] = useState(false);
  const [coords, setCoords] = useState(null);
  const triggerRef = useRef(null);
  const panelRef = useRef(null);
  ...
  const toggle = () => { if (!open) calc(); setOpen((v) => !v); };
  ...
  return (
    <>
      <button type="button" ref={triggerRef} id={id}
        className={"ap-agentsel__trigger" + (open ? " is-open" : "")}
        onClick={toggle} aria-haspopup="listbox" aria-expanded={open}>
        <span className="ap-agentsel__placeholder"><Icon name="user" size={17} />Select agent</span>
        <Icon name="chevron-down" size={17} className="ap-agentsel__chev" />
      </button>

      {open && coords && ReactDOM.createPortal(
        <div ref={panelRef} className="ap-agentsel__drop"
          style={{ top: coords.top, left: coords.left, width: coords.width,
            clipPath: coords.clipTop ? `inset(${coords.clipTop}px 0 0 0)` : undefined }} role="listbox">
          {AGENTS.map((a) => (
            <button key={a.id} type="button"
              className={"ap-agentsel__row" + (value === a.id ? " is-selected" : "")}
              role="option" aria-selected={value === a.id}
              ...
              {value === a.id && <span className="ap-agentsel__check"><Icon name="check" size={16} strokeWidth={2.5} /></span>}
            </button>
          ))}
        </div>,
        document.body
    ...
    <div className="pp-stage" ref={stageRef} data-screen-label="Admin · Property published">
      <section className="pp-card" aria-labelledby="pp-title">
        ...
        <div className="pp-actions">
          <Button hierarchy="primary" size="lg" iconLeading="eye"
            href="Admin-Property Details.html">View property</Button>
          <div className="pp-actions__row">
            <Button hierarchy="secondary" size="lg" iconLeading="plus"
              href="Admin-Add Property page.html">Add another property</Button>
          </div>
          <div className="pp-actions__back">
            <Button hierarchy="link" size="lg" iconLeading="arrow-left"
...
/* Existing property record being edited — every field pre-filled. This is the
   pristine baseline; any divergence from it marks the form as having unsaved
   changes. (Mirrors the published "Olive Grove Estate" demo listing.) */
const INITIAL = {
  listing: "sale",
  type: "Villa",
  title: "Olive Grove Estate",
...
function EditPropertyPage() {
  ...
  /* ---- unsaved-changes detection across the whole flow ---- */
  const pristineRef = useRef(JSON.stringify(INITIAL));
  const savedRef = useRef(false); // set true on Update so the unsaved-changes guard doesn't block the return navigation
  const dirty = JSON.stringify(f) !== pristineRef.current;
  const [pendingNav, setPendingNav] = useState(null); // href awaiting confirmation
  const [toastOpen, setToastOpen] = useState(false);

  // expose dirtiness + a leave-request hook to the shell (sidebar / topbar)
  useEffect(() => {
    ...
    return () => { leaveGuard.dirty = () => false; leaveGuard.requestLeave = (h) => { window.location.href = h; }; };
  }, [dirty]);

  // native guard for refresh / browser-level navigation
  useEffect(() => {
    const onBeforeUnload = (e) => { if (dirty && !savedRef.current) { e.preventDefault(); e.returnValue = ""; } };
    window.addEventListener("beforeunload", onBeforeUnload);
    return () => window.removeEventListener("beforeunload", onBeforeUnload);
  }, [dirty]);

  // in-page guarded navigation (breadcrumb, Cancel)
  const guardedNav = (href) => { if (dirty) setPendingNav(href); else window.location.href = href; };

  const onUpdate = () => {
    pristineRef.current = JSON.stringify(f); // saved data becomes the new baseline
    savedRef.current = true;                 // allow leaving without the unsaved-changes prompt
    // Return to wherever the edit was launched from, and show the success toast there.
    const params = new URLSearchParams(window.location.search);
    const from = params.get("from");
    const id = params.get("id");
    let dest;
    if (from === "details") {
      dest = "Admin-Property Details.html?toast=updated" + (id ? "&id=" + encodeURIComponent(id) : "");
    } else {
      dest = "Admin-Properties Page.html?toast=updated";
    }
  ...
  return (
    <div className="ap-wrap" data-screen-label="Admin · Edit property">
      {/* breadcrumbs */}
      <nav className="ap-crumbs" aria-label="Breadcrumb">
        <a href="Admin-Properties Page.html" onClick={(e) => { e.preventDefault(); guardedNav("Admin-Properties Page.html"); }}>
          <Icon name="building-2" size={14} />Properties
        </a>
        <span className="ap-crumbs__sep"><Icon name="chevron-right" size={14} /></span>
        <a href="Admin-Property Details.html" onClick={(e) => { e.preventDefault(); guardedNav("Admin-Property Details.html"); }}>
          {rev.title}
        </a>
        <span className="ap-crumbs__sep"><Icon name="chevron-right" size={14} /></span>
        <span className="ap-crumbs__current" aria-current="page">Edit property</span>
      </nav>
      ...
      <section className="ap-card">
        ...
        <div className="ap-card__body">
          <div className="ap-grid">
            ...
            <div className="ap-field ap-col-full">
              ...
              <Dropdown id="ap-type" placeholder="Select property type"
        ...
        <div className="ap-foot">
          <Button hierarchy="tertiary" size="lg" iconLeading="arrow-left" onClick={() => guardedNav("Admin-Properties Page.html")}>Cancel</Button>
        ...
        <div className="ap-card__body">
          <div className="ap-grid">
            ...
            <div className="ap-field">
              ...
              <Dropdown id="ap-city" placeholder="Select city"
                value={f.city} onChange={(v) => set("city", v)}
            ...
            <div className="ap-field">
              ...
              <Dropdown id="ap-district" placeholder="Select area or district"
          ...
          <div className="ap-sect">
            ...
            <div className="ap-grid">
              ...
              <div className="ap-field">
                ...
                <Dropdown id="ap-orientation" placeholder="Select orientation"
                  value={f.orientation} onChange={(v) => set("orientation", v)}
              ...
              <div className="ap-field">
                ...
                <Dropdown id="ap-condition" placeholder="Select condition"
              ...
              <div className="ap-field">
                ...
                <Dropdown id="ap-furnishing" placeholder="Select furnishing"
                  value={f.furnishing} onChange={(v) => set("furnishing", v)}
            ...
            <div className="ap-grid">
              <div className="ap-field ap-col-full">
                ...
                {(() => {
                  ...
                  return sel
                    ? <AgentSummary agent={sel} onClear={() => set("agent", "")} />
                    : <AgentSelect id="ap-agent" value={f.agent} onChange={(v) => set("agent", v)} />;
                })()}
              </div>
            </div>
          </div>
      ...
      <section className="ap-card">
        <div className="ap-card__head">
          <h2 className="ap-card__title">Review changes</h2>
          <p className="ap-card__desc">Review your updated property information before saving the changes to this listing.</p>
        </div>
        ...
        <div className="ap-foot">
          ...
          <div className="ap-foot__right">
            ...
            <Button hierarchy="primary" size="lg" iconLeading="check" onClick={onUpdate}>Update property</Button>
          </div>
        </div>
      </section>
      )}
      ...
      <Modal
        ...
        footer={
          <React.Fragment>
            <Button hierarchy="secondary" size="md" onClick={() => setPendingNav(null)}>Cancel</Button>
            <Button hierarchy="primary" size="md" onClick={() => { pristineRef.current = JSON.stringify(f); savedRef.current = true; const h = pendingNav; setPendingNav(null); window.location.href = h; }}>Save changes</Button>
          </React.Fragment>
        }>
      ...
      <UpdateToast open={toastOpen} onDismiss={() => setToastOpen(false)} />
    </div>
  );
}

/* ==================================================================
   UPDATE SUCCESS TOAST  (after "Update property")
================================================================== */
function UpdateToast({ open, onDismiss }) {
  if (!open) return null;
  return ReactDOM.createPortal(
    <div className="et-toast" role="status" aria-live="polite">
      <span className="et-toast__ic"><Icon name="circle-check" size={20} strokeWidth={2.25} /></span>
      <div className="et-toast__body">
        <p className="et-toast__title">Property updated successfully</p>
        <p className="et-toast__desc">The property information has been saved.</p>
        <div className="et-toast__actions">
          <button type="button" className="et-toast__btn" onClick={onDismiss}>Dismiss</button>
          <a className="et-toast__btn et-toast__btn--primary" href="Admin-Property Details.html">
            View property<Icon name="arrow-right" size={14} strokeWidth={2.25} />
          </a>
        </div>
      </div>
      <button type="button" className="et-toast__close" aria-label="Dismiss notification" onClick={onDismiss}>
...
function App() {
  ...
  const handleSelect = useCallback((id) => {
    ...
    if (PAGES[id]) { leaveGuard.requestLeave(PAGES[id]); return; }
    setActive(id);
  }, []);
